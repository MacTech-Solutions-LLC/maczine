...
- **No em dashes.** Never write "—". Where the rhythm wants one - a
  pivot, a parenthetical, an appositive - use a plain hyphen with a
  space on each side, the way this bullet just did. Lint fails the
  build on any em dash in the body or frontmatter; it is not optional
  house style, it is a hard constraint on every piece you write.
